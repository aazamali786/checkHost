...
export const useProvideAuth = () => {
  ...
  const login = async (formData) => {
    ...
    try {
      const { data } = await axiosInstance.post('explore/user/login', {
        email,
        password,
      });
      if (data.user && data.token) {
  ...
  const logout = async () => {
    try {
      const { data } = await axiosInstance.get('/explore/user/logout');
      if (data.success) {
        setUser(null);
...
export const useProvidePlaces = () => {
  ...
  const getPlaces = async () => {
    const { data } = await axiosInstance.get('/explore/places');
